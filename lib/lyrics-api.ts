...
export async function listMyLyrics(params?: { projectId?: string }): Promise<LyricSummary[]> {
  ...
  return apiFetchJson<LyricSummary[]>(`/api/v1/lyrics${query}`)
}
...
export async function getLyric(id: string): Promise<Lyric> {
  return apiFetchJson<Lyric>(`/api/v1/lyrics/${id}`)
}
...
export async function listPublicLyrics(params?: {
  ...
  return apiFetchJson<LyricSummary[]>(`/api/v1/lyrics/ready-library${query ? `?${query}` : ""}`)
}

export async function getPublicLyric(id: string): Promise<Lyric> {
  return apiFetchJson<Lyric>(`/api/v1/lyrics/ready-library/${encodeURIComponent(id)}`)
}
...
export async function createLyric(payload: {
  ...
  return apiFetchJson<Lyric>("/api/v1/lyrics", {
    method: "POST",
...
export async function deleteLyric(id: string): Promise<void> {
  await apiFetchJson<string>(`/api/v1/lyrics/${id}`, {
    method: "DELETE",
  })
}
